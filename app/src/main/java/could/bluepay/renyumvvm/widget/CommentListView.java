...
import could.bluepay.renyumvvm.widget.action.DynamicMovementMethod;
...
public class CommentListView extends LinearLayout {
    ...
    protected void initAttrs(AttributeSet attrs) {
        ...
        try {
            //textview的默认颜色
            itemColor = typedArray.getColor(R.styleable.PraiseListView_item_color, getResources().getColor(R.color.font_color_3));
    ...
    private View getView(final int position){
        ...
        View convertView = layoutInflater.inflate(R.layout.item_dynamic_comment, null, false);

        TextView commentTv = (TextView) convertView.findViewById(R.id.commentTv);
        final DynamicMovementMethod circleMovementMethod = new DynamicMovementMethod(itemSelectorColor, itemSelectorColor);

        final CommentItem bean = mDatas.get(position);
        String name = bean.getUser().getNick_name();
        long id = bean.getId();
        String toReplyName = "";
        if (bean.getToReplyUser() != null) {
            toReplyName = bean.getToReplyUser().getNick_name();
        }

        SpannableStringBuilder builder = new SpannableStringBuilder();
        builder.append(setClickableSpan(name, bean.getUser().getQid()));

        if (!TextUtils.isEmpty(toReplyName)) {

            builder.append(" 回复 ");
            builder.append(setClickableSpan(toReplyName, bean.getToReplyUser().getQid()));
        }
        builder.append(": ");
        //转换表情字符
        String contentBodyStr = bean.getContent();
        builder.append(AppUtils.formatUrlString(contentBodyStr));
    ...
    private SpannableString setClickableSpan(final String textStr, final long id) {
        ...
        subjectSpanText.setSpan(new SpannableClickable(itemColor){
                                    ...
                                    public void onClick(View widget) {
                                        Toast.makeText(MixApp.getContext(), textStr + " &id = " + id, Toast.LENGTH_SHORT).show();
                                    }
