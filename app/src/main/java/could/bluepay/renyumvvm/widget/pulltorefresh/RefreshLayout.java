...
//    public static final int DEFAULT = MaterialProgressDrawable.DEFAULT;
    ...
    private SwipeRefreshDirection mDirection;
    ...
    public RefreshLayout(Context context, AttributeSet attrs) {
        ...
        final TypedArray a2 = context.obtainStyledAttributes(attrs, R.styleable.PullToRefreshView);
        SwipeRefreshDirection direction
                = SwipeRefreshDirection.getFromInt(a2.getInt(R.styleable.PullToRefreshView_ptr_direction, 0));
        if (direction != SwipeRefreshDirection.BOTH) {
            mDirection = direction;
            mBothDirection = false;
        } else {
            mDirection = SwipeRefreshDirection.TOP;
            mBothDirection = true;
        }
        a2.recycle();
        ...
        setColorSchemeResources(android.R.color.holo_blue_bright,
                ...
                android.R.color.holo_red_light);//默认加载颜色
    }
...
//        Log.i("TT","RefreshLayout-->onLayout");
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mBothDirection) {
                    if (y > mInitialDownY) {
                        setRawDirection(SwipeRefreshDirection.TOP);
                    } else if (y < mInitialDownY) {
                        setRawDirection(SwipeRefreshDirection.BOTTOM);
                    }
                    if ((mDirection == SwipeRefreshDirection.BOTTOM && canChildScrollDown())
                            || (mDirection == SwipeRefreshDirection.TOP && canChildScrollUp())) {
                        mInitialDownY = y;
                        return false;
                    }
                }
            ...
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE: {
                    ...
                    if (mIsBeingDragged) {
                        ...
                        int targetY;
                        if (mDirection == SwipeRefreshDirection.TOP) {
...
//        }
    ...
    public interface OnRefreshListener {
        void onRefresh(SwipeRefreshDirection direction);
    }

    public SwipeRefreshDirection getDirection() {
        return mBothDirection ? SwipeRefreshDirection.BOTH : mDirection;
    }

    public void setDirection(SwipeRefreshDirection direction) {
        if (direction == SwipeRefreshDirection.BOTH) {
            mBothDirection = true;
        } else {
            mBothDirection = false;
    ...
    // only TOP or Bottom
    private void setRawDirection(SwipeRefreshDirection direction) {
