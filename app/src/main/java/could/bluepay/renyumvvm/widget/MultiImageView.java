...
public class MultiImageView extends LinearLayout {
	...
	// 照片的Url列表
	private List<PhotoInfo> photoInfoListList;
	...
	private int pxImagePadding = ViewUtils.dp2px(getContext(), 3);// 图片间的间距
...
//	private WeiboBean mDynamicItem;
	...
	public void setList(List<PhotoInfo> lists) throws IllegalArgumentException {
		...
		photoInfoListList = lists;
		imagesUrlList.clear();
		for(PhotoInfo info:photoInfoListList){
			imagesUrlList.add(info.url);
		}
...
//	}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		if(MAX_WIDTH == 0){
			...
			if(width>0){
				MAX_WIDTH = width;
				if(photoInfoListList!=null && photoInfoListList.size()>0){
					setList(photoInfoListList);
				}
			}
		}
	...
	private ImageView createImageView(int position, final boolean isMultiImage) {
		PhotoInfo photoInfo = photoInfoListList.get(position);
		...
		if(isMultiImage){
            ...
            int expectW = photoInfo.width;
            int expectH = photoInfo.height;
