...
import could.bluepay.renyumvvm.databinding.ItemLvInviteBinding;
...
public class UserListInviteAdapter extends  RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public UserListInviteAdapter(Activity context) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        switch (viewType){
            ...
            case TYPE_CONTENT_BOOK:
                ItemLvInviteBinding mBindContent =  DataBindingUtil.inflate(LayoutInflater.from(parent.getContext()),R.layout.item_lv_invite,parent,false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof UserListInviteAdapter.HeaderViewHolder){
            ((UserListInviteAdapter.HeaderViewHolder)holder).bindItem();

        }else if(holder instanceof UserListInviteAdapter.FootViewHolder){
            ((UserListInviteAdapter.FootViewHolder)holder).bindItem();

        }else if(holder instanceof UserListInviteAdapter.UserViewHolder){
            if(userData!=null && userData.size()>0) {
                UserBeanItem item = userData.get(position-1);
                ((UserListInviteAdapter.UserViewHolder) holder).bindItem(item);
            }
        }
    }
